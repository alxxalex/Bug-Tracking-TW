...
const updateBugStatus = async (req, res) => {
  try {
    ...
    if (!id || !status) {
      return res.status(400).json({ message: 'Bug ID and new status are required' });
    }
...
const updateBugSolvedCommitLink = async (req, res) => {
  try {
    ...
    if (!id || !commitLinkBugSolved) {
      return res.status(400).json({ message: 'Bug ID and new commmit link are required' });
    }
...
export { insertBugIntoDb, getBugsFromDb, getBugsForProject, updateBugStatus, updateBugSolvedCommitLink };
